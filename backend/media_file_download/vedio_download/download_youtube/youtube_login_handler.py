...
COOKIES_PATH = Path(__file__).parent / "datas" / "youtube_cookies.json"
...
def cookies_to_netscape(cookies: List[Dict], dest: str) -> bool:
    ...
    """
    try:
        lines = ["# Netscape HTTP Cookie File\n", "# This file was generated by youtube_login_handler\n\n"]
        for c in cookies:
            domain = c.get('domain', '')
            ...
            if expires_val is None or expires_val < 0:
                expires_val = 9999999999
...
async def login_and_get_cookies(headless: bool = False) -> Optional[List[Dict]]:
    """
    使用playwright驱动浏览器登录youtube并获取cookie
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            """)
            logger.info("正在打开youtube登录页面...")
            await page.goto("https://www.youtube.com/signin", wait_until="networkidle")

            logger.info("请在浏览器中完成登录操作（输入账号密码，可能需要验证码）")
            logger.info("登录成功后，程序将自动检测并保存cookie")

            # 等待登录完成：监控URL变化到YouTube主页，或超时
            ...
            while elapsed < timeout:
                ...
                if "youtube.com" in current_url and "signin" not in current_url:
                    logger.info("检测到登录成功，URL已变为: " + current_url)
                    break
                await page.wait_for_timeout(check_interval)
                elapsed += check_interval
            else:
                logger.warning("等待登录超时，可能登录未完成")
