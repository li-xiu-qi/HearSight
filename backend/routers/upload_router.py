...
from backend.services.upload_service import create_audio_placeholder, get_unique_filename
...
async def upload_file(
    ...
    try:
        ...
        if file_ext not in SUPPORTED_VIDEO_FORMATS and file_ext not in SUPPORTED_AUDIO_FORMATS:
            raise HTTPException(
                status_code=400,
...
async def rename_file(
    ...
    try:
        ...
        if not old_filename or not new_filename:
            raise HTTPException(status_code=400, detail="old_filename 和 new_filename 都是必填项")
        ...
        if not new_file_ext:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"不能更改文件扩展名，原扩展名: {file_ext}"
            )
        ...
        try:
            # 更新transcripts表
            transcript_count = update_transcript_media_path(db_url, old_media_path, new_media_path)
            if transcript_count > 0:
                logger.info(f"已更新 {transcript_count} 条转写记录的media_path")
            
            # 更新jobs表的result字段
            job_count = update_job_result_paths(db_url, old_filename, final_filename, str(static_dir))
            if job_count > 0:
        ...
        if is_audio:
            ...
            if old_placeholder.exists():
                new_placeholder = static_dir / f"{Path(final_filename).stem}_placeholder.jpg"
