...
**快速提示**：项目支持使用百度AI Studio的OpenAI兼容API作为LLM服务，可使用ernie-4.5-turbo-128k-preview模型。配置AI_STUDIO_API_KEY（对应OpenAI SDK的api_key字段）和base_url="<https://aistudio.baidu.com/llm/lmapi/v3>"（对应OpenAI SDK的base_url字段）即可。更具体的内容可以参考：<https://ai.baidu.com/ai-doc/AISTUDIO/rm344erns>。仅配置 `OPENAI_API_KEY` 和 `DASHSCOPE_API_KEY` 即可启动全部功能。我们默认使用硅基流动的 OpenAI 兼容 API。免费额度申请：<https://cloud.siliconflow.cn/i/FcjKykMn>，平台免费提供包括文心 4.5 多模态模型的大语言模型调用。DASHSCOPE_API_KEY 可从阿里云百炼获取。
