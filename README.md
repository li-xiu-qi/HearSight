...
- `GET /api/transcripts/{transcript_id}` - 获取转录详情（包含已保存的总结和翻译）
- `POST /api/summarize` - 生成内容摘要
- `POST /api/transcripts/{transcript_id}/summaries` - 保存生成的总结到数据库
- `GET /api/transcripts/{transcript_id}/summaries` - 获取已保存的总结
- `POST /api/transcripts/{transcript_id}/translate` - 开始翻译任务
- `GET /api/transcripts/{transcript_id}/translate-progress` - 获取翻译进度
- `GET /api/transcripts/{transcript_id}/translations` - 获取已保存的翻译
...
支持将转写内容翻译为多种语言（中文、英文等）。

**新增功能**：
- ✅ 翻译结果自动保存到数据库，支持多语言同时保存
- ✅ 支持强制重新翻译，自动更新数据库中的翻译结果
- ✅ 前端显示已保存的翻译语言
- ✅ 可查看之前翻译的内容
